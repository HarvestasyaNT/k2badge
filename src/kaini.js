...
$(document).ready(function() {
    ...
    var saveAll = function() {
        ...
        store("fleet", btoa(fleets.join("|")));
        store("fleetLvl", btoa(fleetLevels.join("|")));
        store("colle", btoa(JSON.stringify(colle)));
        store("bg", globalbg);
    ...
    var loadAll = function() {
        ...
        k2Temp = k2Temp ? JSON.parse(atob(k2Temp)) : null;
        var colleTemp = restore("colle");
        colleTemp = colleTemp ? JSON.parse(atob(colleTemp)) : null;
        var furnTemp = restore("ttkFurn");
        furnTemp = furnTemp ? JSON.parse(atob(furnTemp)) : null;
        var fleetsTemp = restore("fleet");
        fleetsTemp = fleetsTemp ? atob(fleetsTemp).split("|") : null;
        for (var i in fleetsTemp) {
            fleetsTemp[i] = fleetsTemp[i].split(",");
        }
        var fleetLevelsTemp = restore("fleetLvl");
        fleetLevelsTemp = fleetLevelsTemp ? atob(fleetLevelsTemp).split("|") : null;
